...
void ProxyStream::OnFetchComplete(const URLFetcher* source, int64_t msec) {
  ...
  CreateSpdyHeadersFromHttpResponse(*headers, kProtocolVersion, &spdy_headers);

  // Body
  std::string body;
  source->GetResponseAsString(&body);

  SendHeadersAndBody(spdy_headers, body);
...
void ProxyStream::ErrorResponse(int error_code,
  ...
  SendHeadersAndBody(spdy_headers, error_message);
