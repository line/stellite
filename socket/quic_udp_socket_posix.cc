...
int QuicUDPSocketPosix::RecvFrom(IOBuffer* buf,
  ...
  DCHECK(!callback.is_null());  // Synchronous operation is not supported
  DCHECK_GT(buf_len, 0);
...
int QuicUDPSocketPosix::InternalConnect(const IPEndPoint& address) {
  ...
  // Or else, connect() does the DatagramSocket::DEFAULT_BIND
...
int QuicUDPSocketPosix::BindToNetwork(
  ...
  // This is a race, but all racers should come out with the same answer so it
  // shouldn't matter.
