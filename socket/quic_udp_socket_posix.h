...
#ifndef STELLITE_SOCKET_SOCKET_POSIX_H_
#define STELLITE_SOCKET_SOCKET_POSIX_H_
...
// Clone of net/udp/udp_socket_posix.h
// net::UDPSocketPosix SO_REUSEPORT is not supported and cannot
// be implemented on the inherited class because file descriptor is a
// private member. To avoid this, we have to clone it.
class NET_EXPORT QuicUDPSocketPosix : public base::NonThreadSafe {
 public:
  QuicUDPSocketPosix(DatagramSocket::BindType bind_type,
                     const RandIntCallback& rand_int_cb,
                     net::NetLog* net_log,
                     const net::NetLog::Source& source);
  virtual ~QuicUDPSocketPosix();
  ...
  // |callback| is the user callback function to call on completion.
  // Returns a net error code, or ERR_IO_PENDING if the IO is in progress.
  // If ERR_IO_PENDING is returned, the caller must keep |buf| and |address|
  // alive until the callback is called.
 ...
 private:
  ...
  class ReadWatcher : public base::MessageLoopForIO::Watcher {
   public:
    explicit ReadWatcher(QuicUDPSocketPosix* socket) : socket_(socket) {}
   ...
   private:
    QuicUDPSocketPosix* const socket_;
  ...
  class WriteWatcher : public base::MessageLoopForIO::Watcher {
   public:
    explicit WriteWatcher(QuicUDPSocketPosix* socket) : socket_(socket) {}
  ...
  DISALLOW_COPY_AND_ASSIGN(QuicUDPSocketPosix);
};

}  // namespace net

#endif  // STELLITE_SOCKET_SOCKET_POSIX_H_