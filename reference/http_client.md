# interface HttpResponseDelegate
> http response에 대한 client callback interface로 http response, stream response, error 에 대한 callback이 정의가 되어 있다. 이 interface는 smart pointer를 사용하도록 강제되어 있어서 life cycle 중 메모리 해제가 자동으로 처리가 된다.
...
## HttpResponseDelegate::OnHttpResponse

##### The http response callback function. it will be called when HttpClient.Request call.
...
> > > size_t: http response body buffer size

> > Returns:

## HttpResponseDelegate::OnHttpStream

##### The http stream callback function. it wall be called more than one on each request when HttpClient.Stream() call.

> void HttpResponseDelegate::OnHttpStream(const HttpResponse& response, const char* data, size_t len)
...
## HttpResponseDelegate::OnHttpError
...
//
// // Server request for client certificate did not contain any types we
// support.
...
//
// // An SSL peer sent us a fatal decrypt_error alert. This typically occurs
// when
...
//
// // The SSL server requires falling back to a version older than the
// configured
...
//
// // (GSSAPI) No Kerberos credentials were available during HTTP
// Authentication.
...
//
// // SPDY server refused the stream. Client should retry. This should never be
// a
// // user-visible error.
...
// // origin, a violation of HTTP Alternative Services specification Section
// 2.1,
// // https://tools.ietf.org/id/draft-ietf-httpbis-alt-svc-06.html#host_auth.
// (ALTERNATIVE_CERT_NOT_VALID_FOR_ORIGIN, -368)
//
...
// // internal error returned from the HttpCache to the HttpCacheTransaction
// that
// // tells the transaction to restart the entry-creation logic because the
// state
...
// // credentials aren't available.  There isn't a way to get them at that
// point.
...
# class HttpClient
> HTTP 요청을 처리하는 client. protocol에 대한 선택은 HttpClientContext의 parameter option에 따라 결정된다.
...
class HttpClient {
 public:
  ...
  // delegate owned in HttpClient
  virtual bool Request(const HttpRequest& request,
                       sp<HttpResponseDelegate> delegate)=0;

  // delegate owned in HttpClient
  virtual bool Request(const HttpRequest& request,
                       sp<HttpResponseDelegate> delegate, int timeout)=0;

  // delegate owned in HttpClient
  virtual bool Stream(const HttpRequest& request,
                      sp<HttpResponseDelegate> delegate)=0;
...
###### Asynchronized HTTP request function, callback function are called only one whene error occured or http response are completly received.

> bool HttpClientContext::Request(const HttpRequest& request, sp\< HttpResponseDelegate\> delegate)
> > Parameters:

> > > HttpRequest: struct of HttpRequest

> > > HttpResponseDelegate: HttpResponseDelegate this instance will callback successed or failed.

> > Returns:

> > > true : async request are successed

> > > false : async request are failed

###### Asynchronized HTTP request function. this function timeout support. that was if timeout seconds are passed, then Timeout function are call on network thread.

> bool HttpClientContext::Request(const HttpRequest& request, sp\< HttpResponseDelegate\> delegate, int timeout)

> > Parameters:

> > > HttpRequest: struct of HttpRequest

> > > HttpResponseDelegate: this object will callback successed or failed.

> > > int: timeout seconds

> > Returns:

> > > true : async http request are successed

> > > false : async http request are failed

## HttpClientContext::Stream

###### Stream() are different to Request(). the difference is a result callback interface. Request response callback was OnHttpResponse, but Stream response callback was OnHttpStream. OnHttpStream resposne callback are called more than one on each request.

> bool HttpClientContext::Stream(const HttpRequest& request, sp\< HttpResponseDelegate\> delegate)

> > Parameters:

> > > HttpRequest: struct of http request

> > > HttpResponseDelegate: this object will callback successed or failed.

> > Returns:

> > > true : async stream request are successed

> > > false : async stream request are failed

###### Same a Stream(request, delegate) with timeout seconds. timeout is read timeout. for example if stream response are very slow but it keep downloading continuous, it timeout are not called until download are complete. but when read event is not called in downloding, after download timeout seconds are passed OnHttpError are called.

> bool HttpClientContext::Stream(const HttpRequest& request, sp\< HttpResponseDelegate\> delegate, int timeout)
> > Parameters:
