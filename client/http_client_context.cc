...
// Maximum receive window size for HTTP/2 sessions and streams.
const int kSpdySessionMaxRecvWindowSize = 15 * 1024 * 1024;  // 15 MB
const int kSpdyStreamMaxRecvWindowSize = 6 * 1024 * 1024;    //  6 MB
 ...
 private:
  ...
  void InitOnNetworkThread() {
    ...
    if (context_params_.using_quic) {
      ...
      params.quic_packet_loss_threshold = 1.0f;
    ...
    if (context_params_.using_spdy) {
      ...
      params.spdy_session_max_recv_window_size = kSpdySessionMaxRecvWindowSize;
      params.spdy_stream_max_recv_window_size = kSpdyStreamMaxRecvWindowSize;
    }
