...
void HttpFetcherCore::SetUploadData(const std::string& upload_content_type,
  ...
  // Empty |upload_content_type| is allowed if the |upload_content| is empty.
...
const ResponseCookies& HttpFetcherCore::GetCookies() const {
  return cookies_;
}
...
void HttpFetcherCore::OnResponseStarted(URLRequest* request) {
  DCHECK_EQ(request, request_.get());
  DCHECK(network_task_runner_->BelongsToCurrentThread());
...
void HttpFetcherCore::StartURLRequestWhenAppropriate() {
  ...
  // the URLFetcher::Delegate that the request has been canceled.
...
void HttpFetcherCore::InformDelegateDownloadProgress() {
  ...
  delegate_task_runner_->PostTask(
      FROM_HERE,
      base::Bind(
          &HttpFetcherCore::InformDelegateDownloadProgressInDelegateThread,
