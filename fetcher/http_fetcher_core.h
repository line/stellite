...
class HttpFetcherCore : public base::RefCountedThreadSafe<HttpFetcherCore>,
 ...
 public:
  ...
  // Starts the load. It's important this does not happen in the constructor
  ...
  // Reports that the received content was malformed (i.e. failed to parse
  // or validate). This makes the throttling logic that does exponential
  // backoff when servers are having problems and treats the current request as
 ...
 private:
  friend class base::RefCountedThreadSafe<HttpFetcherCore>;

  ~HttpFetcherCore() override;
  ...
  // Returns the max value of exponential backoff release time for
  ...
  URLFetcher::RequestType request_type_;  // Type of the request
  URLRequestStatus status_;          // Status of the request
  URLFetcherDelegate* delegate_;     // Object to notify on completion
  // Task runner for creating thread. Used to interact with the delegate.
  ...
  base::FilePath upload_file_path_;  // Path to the file containing POST payload
  ...
  // returns a 500 response. In this case, the exponential backoff
  ...
  // True if the URLFetcher has been canceled.
  bool was_cancelled_;
  ...
  // back to the delegate immediately.
  bool stop_on_redirect_;
  // True when we've actually stopped due to the redirect halted by the above.
  // We use this to ensure that |url_| is set to the redirect destination
  // rather than the originally-fetched URL.
  ...
  // re-execute the request, after the backoff delay has expired.
  // True by default.
  ...
  // Backoff time delay. 0 by default.
