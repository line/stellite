...
C++ client library for use with Chromium's QUIC module. The library has four interfaces, which are:
...
Represents a class that contains the context information required for HTTP requests. It has factory functions to create (handle, manipulate, control) HttpClient. Also, it uses network thread and thus requires explicit initialization and destroyer function.
...
| Initialize | Initializes context on a background thread, such as hostname resolver, SSL config service, certificate verifier, proxy service, protocols. |
...
| ResetCertBundle | Use this function to reset CA certificate when using OpenSSL option for Stellite build. |
| InitVM | Initialization function used in Android system. This must be called when an application begins.|

### Parameter

|Option | Command | Default value | Example |
| --- | --- | --- | --- |
| `using_quic` | Use QUIC protocol. When set to true, connects to QUIC server via QUIC Discovery. When Alt-Svc header is specified as quic="hostname:port", QUIC Discovery establishes a QUIC connection using the specified QUIC server hostname and the UDP port.  | false | using_quic = true |
| `using_spdy` | Use SPDY 3.1 protocol. When set to true, NPN (Next Protocol Negotiation) attempts to establish a SPDY connection.  | false | using_spdy = true |
| `using_http2` | Use HTTP2 protocol. When set to true, NPN (Next Protocol Negotiation) and ALPN (Application Layer Protocol Negotiation) attempt to establish an HTTP2 connection.  | false | using_http2 = true |
| `using_quic_disk_cache` | Store QUIC server info on disk cache. When set to true, file cache is stored in the .http_cache directory within a mobile sandbox. When server info is cached, a client can attempt to make a request to a server with 0-RTT. | false | using_quic_disk_cache = true |
| `proxy_host` | Allow a client to use a proxy. When proxy_host is set to "http://127.0.0.1:9000", attempts to connect to 127.0.0.1 number 9000 proxy server. If the URL used for the request is https://stellite.io, it is the proxy server, not the HTTP client, that makes the connection and processes the request. | "" | proxy_host = "http://127.0.0.1:8080" |
| `origin_to_force_quic_on` | Force to use QUIC protocol. If you specify "stellite.com:443" for this option and make a client request to URL https://stellite.io:443, you can directly to QUIC server. If you want to use this URL, you need to provide stellite.io certificate and key file to QUIC server. | "" | origin_to_force_quic_on = "https://stellite.io:443" |
...
## HttpResponseDelegate
 ...
 Represents an interface that invokes client callbacks for the HTTP response.
...
| Function | Description |
| --- | --- |
| OnHttpResponse()| A callback function invoked asynchronously from network thread when the response to HttpClient::Request() is available (ready). |
| OnHttpStream()| A callback function invoked asynchronously from network thread when the response to HttpClient::Stream() is available (ready). |
...
| --- | --- | --- |
| request_id | int | An identifier used to identify a request for HttpClient::Request(). For example, from which request the response came from. |
...
| HasHeader() | Checks whether the header exists or not |
| GetHeader() | Gets the header value |
| SetHeader() | Sets the header value |
| RemoveHeader() | Removes the header value for a particular key |
| ClearHeader() | Clears the header value for all the keys |
| ToString() | Converts all the header data to a raw-header string |

### Parameter

| Name | Type | Description |
| --- | --- | --- |
| raw_header | std::string | Full-text of the header data. It contains a delimiter and key-value pairs. |
...
| Function | Description |
| --- | --- |
| EnumerateHeaderLines() | Enumerates all the header elements line by line to check their header key and value. |
| EnumerateHeader() | Enumerates a header for a particular key to check its value. |
| HasHeader() | Verifies whether a header value exists for a particular key. |
| raw_headers() | Generates a single string that contains all the header information. |
| Reset() | Resets all the header information.|
...
| name | std::string | Key name of the HTTP header |
| raw_header| std::string| Raw data of the HTTP header |
...
| charset | std::string | The character set of the HTTP response body (for example, UTF-8)|
...
| network_accessed | bool | Whether the response was sent over a network or not (as opposed to a cached response). When true, it is a network response. |
| was_fetched_via_spdy | bool | When true, the response was made via a SPDY protocol. |
| was_npn_negotiated | bool | Whether NPN negotiation was used for the response. |
| was_fetched_via_proxy | bool | Whether the response was fetched via a proxy. |
